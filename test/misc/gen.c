...
void gen_set_and_check_R(mpz_t R, const mpz_t P, size_t NLIMBS)
{
  ...
  // abort if gcd(P,R) is different than one
  // mpz_cmp: returns 1) positive value if op1 > op2; 2) zero if op1 = op2; 3) a negative value if op1 < op2
  mpz_gcd(gcd, P, R);
  assert(mpz_cmp(gcd, one) == 0);

  // clear gcd and one
  mpz_clears(gcd, one, NULL);
}
...
void gen_precompute_RmP(uint64_t *_RmP, const uint64_t *_P, size_t NLIMBS)
{
  ...
  gen_set_and_check_R(R, P, NLIMBS);
...
)
{
  ...
  // P
  gen_load_from_string(*P, Pstr, NLIMBS);

  // RmP
  gen_precompute_RmP(*RmP, *P, NLIMBS);

  // Pm2
  gen_precompute_Pm2(*Pm2, *P, NLIMBS);

  // u0 : (glob_p[0] * u0) mod 2^64 == -1
  gen_precompute_u0(u0, *P);

  // RmodP
  gen_precompute_RmodP(*RmodP, *P, NLIMBS);

  // R2modP
  gen_precompute_R2modP(*R2modP, *P, NLIMBS);
}
...
)
{
  ...
  gen_fprintf(fout, "uint64_t glob_p[NLIMBS] = {", P, NLIMBS, "};");

  gen_fprintf(fout, "uint64_t glob_mp[NLIMBS] = {", RmP, NLIMBS, "};");
  ...
  gen_fprintf(fout, "uint64_t glob_rM[NLIMBS] = {", R2modP, NLIMBS, "};");
}
...
int main(int argc, char **argv)
{
  ...
  if(argc != 4)
  { fprintf(stderr, "usage:   ./gen (j||c) NLIMBS NUMBER\n");
    fprintf(stderr, "example: ./gen j 7 0x0002341f271773446cfc5fd681c520567bc65c783158aea3fdc1767ae2ffffffffffffffffffffffffffffffffffffffffffffffffffffff\n");
    return -1;
  }
  ...
  { gen_print_jasmin(P, RmP, Pm2, RmodP, R2modP, u0, NLIMBS, fout); }
  else
  { gen_print_c(P, RmP, Pm2, RmodP, R2modP, u0, NLIMBS, fout); }
