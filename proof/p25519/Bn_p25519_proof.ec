...
+ proc; simplify.
  ...
  + ecall {1} (R.addcR_ph a{1} b{1} c{1}); wp; skip => /> &m Hc [c r] /= -> ?.
    by rewrite R.bn_carryE 1:/# b2i0 /bn_modulus /=.
qed.
...
+ proc.
  seq 16 1: (#pre /\ cf{1}=c{2} /\ ImplZZ t{1} x{2}).
   ...
   + ecall {1} (R.addcR_ph x{1} (Array4.of_list W64.zero 
     ...
     rewrite R.bn_carryE 1:/# -exprM /bigint_modulus b2i0 /=.
     have ->/=: valR X = 2^255+19.
      by rewrite R.bn2seq /bn_seq /X of_listK.
    by move=> -> ->; rewrite R.bn_modulusE -exprM /=. 
...
+ proc; simplify.
  ...
  + ecall {1} (R.subcR_ph a{1} b{1} c{1}); wp; skip => /> &m Hc [c r] /= -> ?.
    by rewrite R.bn_borrowE 1:/# b2i0 /bn_modulus /=.
qed.
...
module AImpl = {
  ...
  proc reduce(a: R2): R = {
    ...
    (_cf, al) <@ R.Ops.add1R(al, tmp, false);
...
equiv add1_eq:
 M.bn_add1 ~ R.Ops.add1R:
 ={a, b} /\ ! c{2} ==> ={res}.
proof.
proc; simplify.
while ( ={i} /\ 1 <= i{2} <= nlimbs /\
        (cf){1}=(c){2} /\
        R.bnk i{2} a{1} = R.bnk i{2} r{2} /\
 ...
 split; first smt().
 split.
  rewrite !R.bnkS 1..2:/# /= !get_setE 1..2:/# /=.
  rewrite !R.bnk_setO 1..2:/#; congr; smt().
 move=> j Hj1 Hj2; rewrite get_setE 1:/#.
 by rewrite (_:!j = i{2}) 1:/# /= H /#.
wp; skip => /> &2 Hcf.
split.
 split; first by rewrite !R.bnk1.
 move=> j Hj1 Hj2; rewrite get_setE 1:/#.
 by rewrite (_:!j=0) 1:/#.
move => rL i rR ????.
...
ecall {1} (R.add1R_ph al{1} tmp{1} false).
...
have X1: R2.bnk 5 r0 = red256 (R2.bn a{1}).
 rewrite E1.
 by rewrite -R2.bn_mod 1:/# Ea0h /red256 /split256 /= R.bn_modulusE expr0 /= expr0 /#.
move=> [ah1 al1] /= _ Ea1l [_cf1 r1] /=.
have {Ea1l}Ea1l: ImplZZ al1 (R2.bnk 5 r0 %% Zp25519.M).
 have ->: Zp25519.M = modulusR.
  by rewrite R.bn_modulusE /= expr0.
 by rewrite -R2.bn_mod 1:/# R.bn_modulusE modz_dvd_pow 1:/# -R.bn_modulusE.
 ...
 split.
  case: (_cf1) => Ecf.
   ...
   apply bound_abs; split.
    by move: H1; rewrite Ecf eq_sym eqT R.bn_modulusE /= expr0 /#.
   move=> *.
    smt.
   rewrite divz_small; last done.
   move: H1; rewrite Ecf R.bn_modulusE /= expr0 /= eq_sym neqF => H1.
   split; smt.
 by rewrite R.bn_modulusE /= expr0 /= mulrC.
...
case: (_cf1) => Ecf.
 ...
 have Xr: forall r, valR r < W64.modulus => r = Array4.of_list W64.zero [r.[0]; W64.zero; W64.zero; W64.zero].
  move=> r Hr; apply R.bn_inj.
  by move: Hr; rewrite !R.bn2seq /bn_seq /to_list /mkseq -iotaredE /=; smt(W64.to_uint_cmp).
 rewrite (Xr _) 1:/# !R.bn2seq /bn_seq /to_list /mkseq -iotaredE /=.
 rewrite to_uintD_small.
  have <-: valR r1 = to_uint r1.[0].
   by rewrite Xr 1:/# !R.bn2seq /bn_seq /to_list /mkseq -iotaredE.
  smt().
 by rewrite of_uintK.
rewrite !b2i0 /= /reduce.
have ->: r1.[0 <- r1.[0]] = r1.
